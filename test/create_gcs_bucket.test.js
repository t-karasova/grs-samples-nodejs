...
describe('Create gcs bucket for products', () => {
  const projectId = process.env['PROJECT_ID'];
  const bucketName = `${projectId}_${Math.round(Date.now() / 1000)}`;
  const fileName1 = 'products.json';
  const fileName2 = 'products_some_invalid.json';
  ...
  before(async () => {
    ...
    stdout = execSync(`node setup/create_gcs_bucket.js ${bucketName}`, {
      cwd,
    });
  });
